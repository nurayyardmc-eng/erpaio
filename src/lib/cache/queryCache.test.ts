...
describe("hashQuestion", () => {
  it("aynı normalize → aynı hash", () => {
    const a = hashQuestion("Bu ay satışlar?", "tenant1");
    const b = hashQuestion("  bu ay satışlar?  ", "tenant1");
    expect(a).toBe(b);
  });

  it("farklı tenant → farklı hash", () => {
    const a = hashQuestion("test", "tenant1");
    const b = hashQuestion("test", "tenant2");
    expect(a).not.toBe(b);
  });

  it("farklı soru → farklı hash", () => {
    const a = hashQuestion("question 1", "tenant1");
    const b = hashQuestion("question 2", "tenant1");
    expect(a).not.toBe(b);
  });

  it("64-char hex (SHA-256)", () => {
    const h = hashQuestion("test", "tenant1");
