...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev.slice(0, -1),
        { role: "assistant", status: "success", sql: data.sql, results: data.results, columns: data.columns, total: data.total, latencyMs: data.latencyMs },
      ]);
    } catch (err: any) {
      setMessages((prev) => [
        ...prev.slice(0, -1),
                ...
                {msg.status === "success" && (
                  <div>
                    <div style={{ background: "#060A12", border: "1px solid #131A26", borderRadius: 8, padding: 12, marginBottom: 10 }}>
                      <div style={{ fontSize: 9, color: "#3A4558", marginBottom: 6 }}>SQL · {msg.latencyMs}ms · {msg.total} satır</div>
